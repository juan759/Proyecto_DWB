...
public interface RepoProduct extends JpaRepository<Product, Integer>{
	...
	Product findAllByGtin(@Param("gtin") String gtin);

	@Query(value = "SELECT * FROM product WHERE category_id = :category_id AND status = 1", nativeQuery = true)
	Product findByCategory_id(@Param("category_id") Integer category_id);

	@Modifying
	@Transactional
	@Query(value = "UPDATE product SET status = 1 WHERE gtin = :gtin AND status = 0", nativeQuery = true)
